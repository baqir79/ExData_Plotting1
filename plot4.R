...
par(mfrow=c(2,2))
# First
plot(as.POSIXct(paste(myset$Date, myset$Time),format("%Y-%m-%d %H:%M:%S")), myset$Global_active_power, type= "l", xlab = "", ylab= "Global Active Power")
#Second
plot(as.POSIXct(paste(myset$Date, myset$Time),format("%Y-%m-%d %H:%M:%S")), myset$Voltage, type= "l", xlab = "datetime", ylab= "Voltage")
# Third
plot(as.POSIXct(paste(myset$Date, myset$Time),format("%Y-%m-%d %H:%M:%S")), myset$Sub_metering_1, type= "n", xlab = "", ylab= "Energy sub metering")
points(as.POSIXct(paste(myset$Date, myset$Time),format("%Y-%m-%d %H:%M:%S")), myset$Sub_metering_1, type = "l", col="black")
#points(as.POSIXct(paste(myset$Date, myset$Time),format("%Y-%m-%d %H:%M:%S")), myset$Sub_metering_1, type = "l", col="black")
...
#Fourth
plot(as.POSIXct(paste(myset$Date, myset$Time),format("%Y-%m-%d %H:%M:%S")), myset$Global_reactive_power, type= "l", xlab = "datetime", ylab= "Global_reactive_power")
dev.off()
